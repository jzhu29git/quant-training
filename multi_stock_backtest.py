...
# ── 全仓版策略 ──
class SmaCrossFull(bt.Strategy):
...
def backtest_stock(code, name):
    try:
        df = ak.stock_zh_a_hist(symbol=code, period="daily", start_date="20200101", end_date="20260428", adjust="qfq")
        ...
        cerebro.addstrategy(SmaCrossFull)
        ...
        return {'code':code, 'name':name, 'ret':(end/start-1)*100, 'max_dd':dd_val, 'trades':total_trades, 'won':won}
...
print(f'【全仓版】双均线策略回测 {len(stocks)} 只股票 (2020~2026)\n')
...
for code, name in stocks:
    ...
    if r:
        ...
        print(f'{r["ret"]:+.2f}%  交易{r["trades"]}次  胜率{r["won"]}/{r["trades"]}')
    else:
        print('失败')
    time.sleep(1)

print('\n' + '=' * 70)
print('【全仓双均线策略】多品种回测结果')
print('=' * 70)

if all_results:
    df_r = pd.DataFrame(all_results).sort_values('ret', ascending=False)
    print(f'\n{f"代码":>8s}  {"名称":>8s}  {"收益率":>10s}  {"交易次数":>8s}  {"赢/输":>10s}  {"最大回撤":>8s}')
    print('-' * 55)
    for _, r in df_r.iterrows():
        won_pct = f'{(r["won"]/r["trades"]*100):.0f}%' if r['trades'] > 0 else '-'
        ret_s = f'{r["ret"]:+.2f}%' if abs(r['ret']) < 1000 else f'{r["ret"]:+.0f}%'
        print(f'{r["code"]:>8s}  {r["name"]:>6s}  {ret_s:>10s}  {r["trades"]:>8d}  {r["won"]:>3d}/{r["trades"]-r["won"]:<5d}  {r["max_dd"]:>6.1f}%  ', end='')
    ...
    print(f'  正收益: {(df_r["ret"]>0).sum()}/{len(df_r)}')
    print(f'  平均最大回撤: {df_r["max_dd"].mean():.2f}%')
    print(f'  平均交易次数: {df_r["trades"].mean():.0f}')

    df_r.to_csv('multi_stock_backtest_full.csv', index=False, encoding='utf-8-sig')
    print('\n结果已保存: multi_stock_backtest_full.csv')
